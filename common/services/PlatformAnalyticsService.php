...
/**
 * Platform-wide analytics for the admin Reports & Analytics page.
 */
class PlatformAnalyticsService
{
    /**
     * @param array{department?:string,category?:string,status?:string,organization_id?:int} $filters
     */
    public function getDashboardMetrics(?int $fromTs = null, ?int $toTs = null, array $filters = []): array
    {
        $toTs = $toTs ?? time();
        ...
        $appQuery = $this->scopedApplications($fromTs, $toTs, $filters);
        $prevQuery = $this->scopedApplications($prevFromTs, $prevToTs, $filters);
        ...
        $spark = $this->sparkline($fromTs, $toTs, $filters);

        $payload = [
            'total_applications' => $totalApplications,
            'total_students' => (int) \common\models\Student::find()->count(),
            'total_organizations' => (int) Organization::find()->count(),
            'approved' => $approved,
            'rejected' => $rejected,
            'pending' => $pending,
            ...
            'daily' => $this->dailyApplicationTrends($fromTs, $toTs, $filters),
            'pipeline' => $this->pipelineBreakdown($fromTs, $toTs, $filters),
            'by_field' => $this->applicationsByField($fromTs, $toTs, $filters),
            'kpi' => [
    ...
    {
        $apps = $this->scopedApplications($fromTs, $toTs, $filters)
            ->with(['student.user', 'position.organization'])
            ->orderBy(['a.created_at' => SORT_DESC])
            ->all();

        $lines = ['Application ID,Student,Organization,Position,Status,Created'];
        foreach ($apps as $app) {
            $lines[] = implode(',', [
                $app->id,
                '"' . str_replace('"', '""', $app->student->user->username ?? '') . '"',
                '"' . str_replace('"', '""', $app->position->organization->name ?? '') . '"',
    ...
    private function applicationBase()
    {
    ...
    private function scopedApplications(int $fromTs, int $toTs, array $filters)
    {
        $query = $this->applicationBase()
    ...
    private function applyFilters($query, array $filters): void
    {
        ...
        if (!empty($filters['organization_id'])) {
            $query->andWhere(['p.organization_id' => (int) $filters['organization_id']]);
        }
    }
    ...
    private function sparkline(int $fromTs, int $toTs, array $filters, int $points = 12): array
    {
        ...
        for ($i = 0; $i < $points; $i++) {
            ...
            $values[] = (int) (clone $this->scopedApplications($start, $end, $filters))->count();
        }
        return $values;
    }

    private function dailyApplicationTrends(int $fromTs, int $toTs, array $filters = []): array
    {
        $rows = $this->scopedApplications($fromTs, $toTs, $filters)
            ->select([
    ...
    private function pipelineBreakdown(int $fromTs, int $toTs, array $filters = []): array
    {
        $statuses = [
            Application::STATUS_PENDING => 'New',
        ...
        $labels = [];
        $values = [];
        $colors = ['#2563eb', '#6366f1', '#10b981', '#f59e0b', '#38bdf8', '#ef4444', '#94a3b8'];
        $colorSlice = [];
        $i = 0;
        foreach ($statuses as $status => $label) {
            $count = (int) (clone $this->scopedApplications($fromTs, $toTs, $filters))
    ...
    private function applicationsByField(int $fromTs, int $toTs, array $filters = []): array
    {
        $rows = $this->scopedApplications($fromTs, $toTs, $filters)
