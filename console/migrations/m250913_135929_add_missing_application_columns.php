...
class m250913_135929_add_missing_application_columns extends Migration
{
    /**
     * {@inheritdoc}
     */
    public function safeUp()
    {
        ...
        if (!$this->db->getTableSchema('{{%application}}')->getColumn('user_id')) {
            $this->addColumn('{{%application}}', 'user_id', $this->integer()->notNull()->after('id'));
        }
        ...
        $applications = $this->db->createCommand('SELECT id, student_id FROM {{%application}} WHERE user_id IS NULL OR user_id = 0')->queryAll();
        ...
        foreach ($foreignKeys as $fk) {
            if (isset($fk->columns[0]) && $fk->columns[0] === 'user_id') {
                $hasForeignKey = true;
                break;
            }
        }
    ...
    {
        echo "m250913_135929_add_missing_application_columns cannot be reverted.\n";
