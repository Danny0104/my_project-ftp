...
(function () {
  ...
  function initSparklines() {
    qsa('.org-analytics-sparkline').forEach(function (canvas) {
      var values = parseJsonAttr(canvas, 'data-values');
  ...
  function updateDaterangeDisplay() {
    var form = qs('[data-org-analytics-filter]');
    if (!form) return;
    var from = form.querySelector('[name="from"]');
    var to = form.querySelector('[name="to"]');
    var display = qs('.org-analytics-daterange__display');
  ...
  function setLoading(on) {
    var root = qs('#orgAnalyticsRoot');
    if (root) root.classList.toggle('org-analytics--loading', !!on);
  }
  ...
  function initAnalyticsFilters() {
    ...
    var root = qs('#orgAnalyticsRoot');
    if (!form || !root) return;
    if (form.dataset.orgAnalyticsBound === '1') return;
    form.dataset.orgAnalyticsBound = '1';
  ...
  function initExportMenu() {
    var wrap = qs('.org-analytics-export-wrap');
    if (!wrap || wrap.dataset.orgExportBound === '1') return;
    wrap.dataset.orgExportBound = '1';
    var toggle = qs('.org-analytics-export-toggle', wrap);
    var menu = qs('.org-analytics-export-menu', wrap);
  ...
  function updateInsights(insights) {
    ...
    if (!insights || !insights.length) {
      grid.innerHTML = '<article class="org-analytics-insight org-analytics-insight--neutral"><span class="org-analytics-insight__icon"><i class="fas fa-chart-line"></i></span><p>No insights for this period.</p></article>';
      return;
    }
    grid.innerHTML = insights.map(function (ins) {
      return '<article class="org-analytics-insight org-analytics-insight--' + ins.type + '">' +
        '<span class="org-analytics-insight__icon"><i class="fas ' + ins.icon + '"></i></span>' +
  ...
  function applyMetrics(metrics) {
    ...
    var cards = qsa('.org-analytics-kpi');
    cards.forEach(function (card, i) {
      var data = metrics.kpi[kpiKeys[i]];
      if (!data) return;
      var counter = card.querySelector('[data-org-counter]');
      if (counter) {
        counter.setAttribute('data-org-counter', data.value);
    ...
    destroyCharts();
    updateCanvas('orgChartFields', metrics.by_field);
    updateCanvas('orgChartDaily', metrics.daily);
    updateCanvas('orgChartPipeline', metrics.pipeline, true);
  ...
  function updateLegend(legend) {
    ...
    if (!legend || !legend.length) {
      ul.innerHTML = '<li class="org-analytics-legend__empty">No applications in this period</li>';
      return;
    }
    ul.innerHTML = legend.map(function (item) {
      return '<li><span class="org-analytics-legend__dot" style="background:' + item.color + '"></span>' +
        '<span class="org-analytics-legend__label">' + item.label + '</span>' +
        '<span class="org-analytics-legend__pct">' + item.pct + '%</span>' +
        '<span class="org-analytics-legend__count">(' + item.count + ')</span></li>';
    }).join('');
  }

  function bootstrapAnalytics() {
    if (!qs('#orgAnalyticsRoot')) return;
    initAnalyticsCharts();
  ...
  document.addEventListener('pjax:end', function () {
    ...
    if (typeof animateCounters === 'undefined') {
      qsa('[data-org-counter]').forEach(function (el) {
        var t = parseInt(el.getAttribute('data-org-counter'), 10) || 0;
